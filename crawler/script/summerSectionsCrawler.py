...
class SummerSectionsCrawler(scrapy.Spider):
    name =            "SummerSectionsCrawler"
    allowed_domains = ['apps.fit.edu']
    start_urls =      ['https://apps.fit.edu/schedule/main-campus/summer']
    ...
    def parse(self, response):
        ...
        for tr in response.xpath('//*[@id="course-table"]/tbody/tr'):
            ...
            # Only summer schedule has session field.
            session = tr.xpath('td[7]/text()').get()
            if session is None:
                session = ''
            ...
            places = [
                [i.strip().split(' ')[0], i.strip().split(' ')[1]]
                for i in tr.xpath('td[10]/text()').extract() if len(i.strip()) > 0
            ]

            instructor = tr.xpath('td[11]/a/text()').get()
            if instructor is None:
                instructor = ''

            cap = [tr.xpath('td[12]/strong/text()').get(), tr.xpath('td[12]/text()').get()[1:]]
